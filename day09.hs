...
compute mem pc rb inp = let [p1, p2, p3, p4] = take 4 (drop (fromIntegral pc) (mem ++ repeat 0)) in
                        ...
                        let [a1, a2, a3] = map (\(p,m) -> getAddress p m rb) [(p2, a), (p3,b), (p4,c)] in
                            case op of
                                ...
                                Output    -> let (mem', out) = compute mem (pc + 2) rb inp in (mem', v1:out)
...
run :: Memory -> Input -> [Value]
run mem inp = snd (compute mem 0 0 inp)

answerA :: [Value]
answerA = run mainInput [1]
...
answerB :: [Value]
answerB = run mainInput [2]

main = do putStrLn (show answerB)
-- answerB = 53088
